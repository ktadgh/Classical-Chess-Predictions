...
class Process:
    ...
    def __init__(self, pgn_file, file_games, file_moves, engine_path, engine_depth, extra_headers):
    ...
    def parse_file(self, add_headers_flag=True):
        ...
        worker = Thread(target=self.__process_move_queue, args=(q,), daemon=True)
        worker.start()
        ...
        if add_headers_flag:
            headers = file_headers_game + self.extra_headers
            game_writer.writerow(headers)
        ...
        while True:
            ...
            game_writer.writerow(self.__get_game_row_data(game, game_id, order, self.pgn_file, self.extra_headers))
    ...
    def __process_move(self, game_id, game, moves_writer, engine, depth):
        ...
        for move in game.mainline_moves():
            ...
            try: index = chess.SQUARE_NAMES.index(player_move.get_to_square())
            #note - add a name for the error here
            except:
    ...
    def __get_game_row_data(self, game, game_id, order, file_name, extra_headers):
        """
        ...
        for header in self.extra_headers:
            if header in game.headers:
                csv_headers.append(game.headers[header])
            else:
                csv_headers.append("")
        return csv_headers
    ...
    def __get_evaluation_from_board(board, depth, engine, is_white_move):
        # this wrapped in a function because engine.analyse is not able to
